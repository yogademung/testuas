...
<body>
	...
	$per_page=5; /* Jumlah Data yang ditampilkan Setiap Page*/
	  $page_query=mysql_query("select COUNT(*) from barang order by id_barang DESC");
	  $pages = ceil(mysql_result($page_query, 0) / $per_page);
	  $page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;
	  $start = ($page - 1) * $per_page;


						$query = mysql_query("SELECT * FROM barang LIMIT $start, $per_page") or die('Kesalahan pada proses query!');
						...
						echo "<hr>";
							...
							echo "<h4>".$data['deskripsi']."</h4>";
							echo "<br>";
							echo "<p>Rp.".$data['harga']."<p>";
							echo "<h4>Kondisi : ".$data['kondisi']."</h4>";
							echo "<br>";
							echo "<a href='deskripsi.php?id_barang=".$id_barang."' class=\"btn btn-default btn-lg\">Detail</a>";
	 ...
	 if($pages >= 1 && $page <= $pages){
	    ...
	    '<center><strong><ul class="pagination pagination-lg"><li><a href="?page='.$x.'">'.$x.'</a></li>' : '<li>
		<a  href="?page='.$x.'">'.$x.'</a></li></ul></strong></center>' ;
	     }
			 }
			 ?>    
	 </div></div></div>
